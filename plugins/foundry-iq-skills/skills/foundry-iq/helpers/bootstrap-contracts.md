...
**Basic/standard Search in an existing group**; prefer MCP.
Other prerequisites/cleanup: [native bootstrap](../references/bootstrap-azure.md).

Apply progress: stderr; `--no-progress` disables.

## Choices

Python (Windows 3.12+), signed-in Azure CLI; no login/install/account switch/up-front policy reads.
Prerequisites: **owner-verified references**, not helper-verified.

Choices: `search`, `create`/`reuse`, region/lowercase SKU.
New locations fold ASCII case/whitespace (`East US` → `eastus`);
normalization alone is not availability proof. Retained bodies/hashes stay unchanged.
...
"prerequisites":{"quota":"observed exact region quota","pricing":"approved price estimate","network":"approved public endpoint","requirements":"owner supplied known required settings","exclusive_name_authority":"owner controls this new name"},
...
required tags must match, never transferring ownership.
...
Labels show caller attestation, not helper verification;
exact references stay private.
Show resource/network/capacity/tags/cost/limits; `--approve` consents once.
No cached consent or user checksum ceremony.
...
Windows MSI/ZIP uses bundled `python.exe -IBm azure.cli`; other layouts block.
...
CLI retries/OS/filesystems prevent hard wall/process-tree/request-count guarantees.
...
Routine checks: target, principal/tenant, capacity/SKU, auth/network, identity,
...
Existing absolute private directory **outside the installed plugin**.
...
Mocked/DrvFS metadata is not native-filesystem or Azure evidence.
...
Message digest/`message_withheld`; no secrets in labels/tags.
