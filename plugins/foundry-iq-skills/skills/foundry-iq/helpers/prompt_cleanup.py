...
try:
    ...
    from .prompt_connect import _connection_url, _load_sdk, _project_identity
except ImportError:
...
def _validate_plan(
    ...
    if agent is not None:
        require_allowed_fields(
            agent,
            {"name", "version", "run_owned", "owned_definition_digest"},
    ...
    *,
    ...
    try:
        ...
        try:
            ...
            try:
                remaining = list(
                    client.agents.list_versions(agent_name=agent["name"])
                )
