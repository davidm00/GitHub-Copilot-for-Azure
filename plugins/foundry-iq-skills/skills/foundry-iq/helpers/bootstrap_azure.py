...
def validate(request):
    ...
    for key, value in request["prerequisites"].items():
        if key == "exclusive_name_authority" and request["action"] == "reuse" and value is None:
            continue
        if not text(value):
            raise failure("bootstrap-prerequisite-missing", "Retain exact quota, price, network, known requirements and exclusive-name evidence.")
...
def plan_request(request, *, cli=run_cli, clock=time.monotonic):
    validate(request)
...
def main(argv=None):
    ...
    try:
        ...
        result = (discover_regions(document) if args.regions else
                  plan_request(document) if args.plan else apply_artifact(
