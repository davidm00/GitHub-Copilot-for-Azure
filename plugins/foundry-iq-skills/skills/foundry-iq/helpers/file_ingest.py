...
def _list_files(
    ...
    *,
    ...
    while current_url:
        ...
        try:
            result = transport(
                "GET", current_url, token, timeout=remaining,
                response_deadline=deadline, max_response_bytes=MAX_INVENTORY_RESPONSE_BYTES,
...
def _confirm_ambiguous_upload(
    ...
    """Resolve one ambiguous upload outcome with a single readback.
    ...
    """
    try:
        observed, observed_request_ids = _list_files(
            list_url, token, transport=transport
        )
    except HelperFailure:
        return None
    request_ids.extend(observed_request_ids)
...
def execute(
    ...
    *,
    ...
    before, request_ids = _list_files(list_url, token, transport=transport)
