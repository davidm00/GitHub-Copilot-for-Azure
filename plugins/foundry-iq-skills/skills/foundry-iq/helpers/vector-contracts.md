...
File/Blob/ADLS and existing models only. No provisioning, catalogs,
role changes, installs, index repairs or cleanup.
Optional: lexical stays a valid default and skips this contract.

## Supported choices

`minimal` is extraction, not retrieval mode. Lexical omits `embeddingModel`:
no source embedding model. Vectors and CU are independent opt-ins for both
...
These embedding guards leave File CU's separate approved API-key ENV channel unchanged;
legacy modes remain.
