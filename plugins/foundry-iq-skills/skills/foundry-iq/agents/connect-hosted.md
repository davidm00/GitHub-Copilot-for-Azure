...
Selected branch of [Connect](connect.md), not an independent creation workflow.
Read before the parent proposes changes. Reuse established context and the
existing source project; do not reinstall, reinitialize or inspect policy upfront.
Use the approved existing `azd ai agent` source project; unsupported/image-only source
blocks rather than selecting another deployment surface.

## Inspect

Read exact scoped resources and compare protected state before any setters:
...
Use the current project/environment without `azd ai project set`. If context
needs changing, include that delta in approval rather than treating it as a read.
...
Compare agent identity/version, source/config hashes, model, telemetry, ACR,
...
Inspect the source-supported mode and the environment actually emitted by the
manifest, not just local variable presence. For `FoundryToolbox`, an explicitly
...
Validate resolution offline with the installed SDK when available; do not
install dependencies solely for this check. No token/model call or standalone
agentic toolbox `tools/list`: runtime identity requires published-agent context.

## Approved changes only

The parent plan includes exact missing grants, resource/binding deltas, source
digest, costs and verification. Exact state skips every mutation below.
...
Pin the observed version when creating the session. A new session does not guarantee a new Responses
conversation. For independent tests, start both fresh:
...
Sessions are already bound to a version at creation. `--version` and `--session-id`
are mutually exclusive; the second call selects the verified session, not a version.
...
Compare before/after identities, versions, tools, roles, protected source and
binding. Repeat reconciliation through reads only: no setters, create, publish
