...
Pass the chosen `--subscription` explicitly. For names/URLs or unresolved resource
choices, use [resource intake](resource-intake.md). Browse relevant resource types
in the known group or selected subscription; useful choices are worth discovery.
...
Choose extraction and indexing independently. Hybrid needs selected embeddings
and their costs. Neither choice determines KB reasoning effort or synthesis.
Existing retrieval remains read-only; missing vector capability needs separately
approved source setup, never a silent configuration change.

| Dependency state | Action |
|---|---|
| Empty | Resolve RG-only, then Search owner; selected models/roles separately |
| RG-only | Reuse group; Search owner resolves selected service |
| Search-only | Honor reuse/new choice; otherwise offer recommendation/options once |
...
Read exact dependency IDs first. Enumerate only unresolved selections within the chosen
...
pre-write checks.
Search owner handles explicit reuse/new selection, exact absence and material
region/SKU/identity/network/local-auth/tags/capacity checks.
