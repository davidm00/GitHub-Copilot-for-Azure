...
Here: RG/models/roles, File CU and separately approved cleanup.
...
Offer choices/cost/access;
ask only unresolved choices, not discovery outputs.
Enumerate only unresolved selections: `az group list --subscription
<sub>` if no group is selected. Only model-dependent paths with no
selected account use `az cognitiveservices account list --subscription <sub>`.
Add a known `--resource-group <group>`; otherwise browse the selected subscription.
Read every page within the selected scope.
Only target ResourceNotFound proves absence, not 403/timeouts/empty output.

For account/deployment selection or new deployments, read
[model choices](resource-intake.md#existing-model-choices).
Exact existing deployments need readback, not catalog searches.
Minimal lexical paths without model dependencies skip model calls.
...
URLs are `https://management.azure.com<ID>?api-version=<version>`.
...
| A/deployments/name / `2025-06-01` | `{"sku":{"name":"<deployment-SKU>","capacity":1},"properties":{"model":{"format":"OpenAI","name":"<model>","version":"<version>"}}}` |

Use required tags/settings; propose capacity/replicas.
Poll GET to `provisioningState == Succeeded`; compare ID/body/tags and read
principalId/account endpoint/model/version/SKU/capacity. Accepted is not ready.
...
Only standard File loads [CU setup](standard-cu.md); minimal never loads it.
